...
1. Login ด้วย admin
2. ไปเมนู **นำเข้าสต๊อก** — ระบบจะโหลดข้อมูลทั้งหมดจาก Database มาแสดงเป็น **แท็บรหัสสี** (เหมือน sheet tabs):
   - **แต่ละแท็บ = รหัสสี 1 รหัส** สลับไปมาได้อิสระ ข้อมูลที่กรอกค้างไว้ไม่หายจนกว่าจะรีเฟรช
   - แต่ละแถวกรอก: **กว้าง, สูง, จำนวนผืน, ประเภทผ้าม่าน** (ตาไก่/ตะขอ/ตีนตุ๊กแก ฯลฯ), **การต่อผ้า** (ไม่ต่อ/ต่อล่าง/ต่อข้าง/ต่อล่าง+ข้าง)
   - **+ เพิ่มรหัสสี** เพื่อสร้างแท็บใหม่ · **✏️ เปลี่ยนรหัส** · **🗑️ ลบแท็บ**
   - กด **Enter** ที่ช่องสุดท้ายเพื่อขึ้นแถวใหม่ · **วางจาก Excel/Sheet ได้** (Ctrl+V หลายแถวรวด)
   - **↕️ เรียงตามขนาด** — เรียงกว้างน้อย→มาก แล้วสูงน้อย→มาก อัตโนมัติ
   - แก้ไขข้อมูลเดิมได้ทันทีในตาราง
   - กด **💾 บันทึกทั้งหมด** ครั้งเดียว → ทุกแท็บที่แก้ไข (มีจุดสีส้ม) บันทึกขึ้น Database พร้อมกัน
3. เมนู **ผู้ใช้งาน** ใช้เพิ่มบัญชีพนักงาน (staff) และเซลส์ (sales)

> **ขนาดซ้ำกันได้** — รหัสสี+ขนาดเดียวกันใส่ได้หลายแถว (เช่น A6 1.20×1.20 คนละล็อต/คนละเฉด) ระบบไม่รวมให้อัตโนมัติ แต่ละแถวเป็นอิสระ
> **บันทึก = เขียนทับทั้งรหัสสีนั้น** ด้วยข้อมูลในตาราง ดังนั้นถ้าลบแถวออกแล้วบันทึก แถวนั้นก็จะหายจาก Database ด้วย
...
> **การควบคุมสถานะการเบิก** (เปิด/ปิด, ตั้งเวลาปิด, ประกาศ) ย้ายไปอยู่ที่ **แถบด้านบน (header)** แล้ว — staff/admin เห็นและใช้ได้จากทุกหน้า
