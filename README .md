...
1. Login ด้วย admin
2. ไปเมนู **นำเข้าสต๊อก** — กรอกข้อมูลแบบตาราง (เหมือน Google Sheet) ได้เลย:
   - **รหัสผ้าม่าน** (เช่น 734, 828-1, A4, 2021-5, Z05)
   - **กว้าง (ซม.)**, **สูง (ซม.)**, **จำนวน (ม้วน)**
   - กด **Tab** ข้ามช่อง, กด **Enter** ที่ช่องจำนวนเพื่อเพิ่มแถวใหม่
   - **วางข้อมูลจาก Excel/Google Sheet** ได้เลย (คัดลอกหลายแถวแล้ว Ctrl+V ในตาราง)
   - กรอกครบแล้วกด **บันทึกนำเข้าทั้งหมด** — จบในหน้าเดียว ไม่ต้อง sync
3. เมนู **ผู้ใช้งาน** ใช้เพิ่มบัญชีพนักงาน (staff) และเซลส์ (sales)

> ระบบจะรวมจำนวนอัตโนมัติถ้ารหัส+ขนาดเดียวกันถูกนำเข้าซ้ำ (เช่น 734 ขนาด 150×160 นำเข้า 45 แล้วนำเข้าอีก 5 → รวมเป็น 50)
